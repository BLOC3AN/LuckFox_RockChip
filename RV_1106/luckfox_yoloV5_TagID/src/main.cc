...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < detect_result_group.count; i++)
  {
    ...
    for(int x = x1; x <= x2; x++) {
        if(y1 >= 0 && y1 < img_height) {
            int idx = (y1 * img_width + x) * 3;
            input_data[idx] = 255; input_data[idx+1] = 0; input_data[idx+2] = 0;
        }
        if(y2 >= 0 && y2 < img_height) {
            int idx = (y2 * img_width + x) * 3;
            input_data[idx] = 255; input_data[idx+1] = 0; input_data[idx+2] = 0;
        }
    }
    for(int y = y1; y <= y2; y++) {
         if(x1 >= 0 && x1 < img_width) {
            int idx = (y * img_width + x1) * 3;
            input_data[idx] = 255; input_data[idx+1] = 0; input_data[idx+2] = 0;
        }
        if(x2 >= 0 && x2 < img_width) {
            int idx = (y * img_width + x2) * 3;
            input_data[idx] = 255; input_data[idx+1] = 0; input_data[idx+2] = 0;
        }
    }
  }
  ...
  // Save image
  stbi_write_jpg("out.jpg", img_width, img_height, 3, input_data, 100);
  printf("Saved result to out.jpg\n");
